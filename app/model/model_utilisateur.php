<?php
    ...
    function getUtilisateurByMail(PDO $bdd,string $email):array|bool{
        try {
            $requete = $bdd->prepare('SELECT id_utilisateur FROM utilisateur WHERE mail_utilisateur = ?');
    ...
    function insertUtilisateur(PDO $bdd, string $nom, string $prenom, string $email, string $password, string $image, int $id_roles):void{
        try {
